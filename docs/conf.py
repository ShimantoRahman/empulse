...
print(sys.path)
import empulse  # ruff: ignore[module-import-not-at-top-of-file]
...
def generate_versions_json():
    if 'dev' in release:
        ...
        versions = [
            {'name': 'dev (latest)', 'version': release, 'url': 'https://empulse.readthedocs.io/en/latest/'},
            {
...
import numpydoc.validate as _npv  # ruff: ignore[module-import-not-at-top-of-file]


@property  # type: ignore[misc]
def _safe_validator_name(self):
    obj = self.obj
...
# The literature reference is a maintainer note on where each dataset and cost matrix comes from,
# not part of the published documentation.
exclude_patterns = ['_build', 'Thumbs.db', '.DS_Store', 'reference/datasets_literature_reference.md']
